 ...
 *    files the node under `incomplete`. The result panels sit on
 *    `--color-overlay`, a translucent wash over the page, so their text
 *    composites onto whatever is behind rather than a flat colour.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Note this lab's entrance
 *    fades live only in `@keyframes`, and reduced motion collapses them to
 *    0.001ms, so nothing strands at `opacity: 0` under the preference.
 *
 ...
 *    the hex envelopes and 3DH transcript messages are long and monospace, so at
