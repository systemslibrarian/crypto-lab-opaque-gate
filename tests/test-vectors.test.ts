 ...
 *
 * These are Known-Answer Tests (KATs): they run under `npm test` (vitest)
 * and in CI, so a regression in any derivation fails the build.
 */
...
import { oprfBlindEvaluate, oprfFinalize } from '../src/oprf';
import {
  store,
  deriveOprfKey,
  RegistrationRecord
...
/**
 * Assert two byte arrays are equal, reporting a hex diff on failure so a
 * drifted derivation is easy to locate.
 */
...
describe('RFC 9807 Appendix C P256-SHA256 Real vector (16 byte-for-byte checks)', () => {
  it('matches every intermediate and output of the CFRG vector', async () => {
    ...
    expectBytes(concat(regEvaluated, V.server_public_key), V.registration_response); // 3
    ...
    expectBytes(randomizedPwd, V.randomized_password); // 4

    // --- Envelope-derived keys ---
    const maskingKey = expand(randomizedPwd, new TextEncoder().encode('MaskingKey'), Nh);
    expectBytes(maskingKey, V.masking_key); // 5

    const authKey = expand(
      randomizedPwd,
      concat(V.envelope_nonce, new TextEncoder().encode('AuthKey')),
      Nh
    );
    expectBytes(authKey, V.auth_key); // 6

    // --- Client static keypair from envelope_nonce ---
    const seed = expand(
      randomizedPwd,
      concat(V.envelope_nonce, new TextEncoder().encode('PrivateKey')),
      Nseed
    );
    const { publicKey: clientPub } = deriveDiffieHellmanKeyPair(seed);
    expectBytes(clientPub, V.client_public_key); // 7
    ...
    expectBytes(storeOut.envelope, V.envelope); // 8
    expectBytes(storeOut.exportKey, V.export_key); // 9
    ...
    expectBytes(regUpload, V.registration_upload); // 10
    ...
    expectBytes(serializeKE1(ke1), V.KE1); // 11
    ...
    expectBytes(ke2Bytes, V.KE2); // 12

    const { ke3, sessionKey, exportKey: loginExportKey } = await clientLoginStep3(ke2, clientState);
    expectBytes(ke3.clientMac, V.KE3); // 13
    expectBytes(loginExportKey, V.export_key); // 14 (login export_key)
    expectBytes(sessionKey, V.session_key); // 15

    const serverSessionKey = await serverFinalize(ke3, serverState);
    expectBytes(serverSessionKey, V.session_key); // 16 (server session_key)
  });
});
...
describe('RFC 9807 Appendix C.2 P256-SHA256 Fake-login vector (1 byte-for-byte check)', () => {
  it('reproduces the fabricated KE2 for an unknown credential', async () => {
    ...
    expectBytes(ke2Bytes, VF.KE2); // 17
  });
});
